...
do
	...
	echo "#!/bin/bash" > atari_pooling_mono_$GAME.cmd
	echo "#SBATCH -J atari-pooling-mono-$GAME" >> atari_pooling_mono_$GAME.cmd
	echo "#SBATCH -N 1" >> atari_pooling_mono_$GAME.cmd
	echo "#SBATCH -n 1" >> atari_pooling_mono_$GAME.cmd
	echo "#SBATCH --ntasks-per-node=1" >> atari_pooling_mono_$GAME.cmd
	echo "#SBATCH --ntasks-per-core=1" >> atari_pooling_mono_$GAME.cmd
	echo "#SBATCH -o atari-pooling-mono-$GAME.out" >> atari_pooling_mono_$GAME.cmd
	echo "#SBATCH -e atari-pooling-mono-$GAME.log" >> atari_pooling_mono_$GAME.cmd
	echo "#SBATCH --time=00:05:00" >> atari_pooling_mono_$GAME.cmd
	echo "#SBATCH --mail-user=erwanlecarpentier@mailbox.org" >> atari_pooling_mono_$GAME.cmd
	echo "#SBATCH --mail-type=END" >> atari_pooling_mono_$GAME.cmd
	echo "" >> atari_pooling_mono_$GAME.cmd
	echo "julia --threads 1 --project=/users/p21001/lecarpen/IICGP.jl scripts/atari_dualcgp.jl --cfg=cfg/test_dualcgp_atari_pooling.yaml --game=$GAME" >> atari_pooling_mono_$GAME.cmd

	sbatch atari_pooling_mono_$GAME.cmd

	rm atari_pooling_mono_$GAME.cmd
done
